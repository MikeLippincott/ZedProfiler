"""Tests for volumesizeshape module compute behavior."""
...
def test_compute_no_args_returns_empty_schema() -> None:
    ...
    result = volumesizeshape.compute()
...
def test_compute_requires_both_loaders() -> None:
    ...
    with pytest.raises(ZedProfilerError):
        volumesizeshape.compute(image_set_loader=DummyImageSetLoader())

    with pytest.raises(ZedProfilerError):
        volumesizeshape.compute(object_loader=DummyObjectLoader())
...
def test_compute_delegates_when_loaders_provided(
    ...
    monkeypatch.setattr(volumesizeshape, "measure_3D_volume_size_shape", _fake_measure)
...
def test_empty_feature_result_schema() -> None:
    ...
    result = volumesizeshape._empty_feature_result()
...
def test_get_skimage_measure_import() -> None:
    ...
    try:
        measure = volumesizeshape._get_skimage_measure()
...
def test_get_skimage_measure_missing_raises_error(
    ...
    monkeypatch.setattr(volumesizeshape, "import_module", mock_import_module)

    with pytest.raises(ZedProfilerError, match="scikit-image"):
        volumesizeshape._get_skimage_measure()
...
def test_calculate_surface_area_with_simple_data(
    ...
    try:
        result = volumesizeshape.calculate_surface_area(label_object, props, spacing)
...
def test_measure_3d_volume_size_shape_returns_dict(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test that measure_3D_volume_size_shape returns correct structure."""
    ...
    with pytest.raises(ZedProfilerError):
        volumesizeshape.measure_3D_volume_size_shape(
            DummyImageSetLoader(),
            DummyObjectLoader(),
        )


def test_measure_3d_volume_size_shape_with_real_data() -> None:
    """Test measure_3D_volume_size_shape with real scikit-image data."""
    try:
        ...
        result = volumesizeshape.measure_3D_volume_size_shape(
            RealImageSetLoader(),
...
def test_measure_3d_volume_size_shape_empty_objects() -> None:
    """Test measure_3D_volume_size_shape with empty object list."""
    try:
...
def test_measure_3d_volume_size_shape_with_anisotropic_spacing() -> None:
    """Test measure_3D_volume_size_shape with anisotropic (non-uniform) spacing."""
    try:
